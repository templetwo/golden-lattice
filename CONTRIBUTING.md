...
- **Tests exercise refusals, not happy paths.** The suite is organized around the question: *can this collapse mode happen if someone tries to construct it?* New tests should follow the same pattern. The happy paths are obvious. The refusals are what the system is. Do not hard-code a test count in docs — run `pytest -q` for the current total.
