...
def build_phase_3_dialogue_prompt(
    *,
    ...
    system = (
        ...
        "\n"
        f"  - converge: name where alignment exists. Use converge in two distinct "
        f"ways. Claim-specific convergence: when you genuinely agree with specific "
        f"claims a peer made, name target_model and target_claim_ids. General "
        f"convergence: when you recognize alignment with the overall framing or "
        f"argument shape rather than specific claims, omit target_claim_ids and "
        f"describe the alignment in your content. Both forms are valid; choose "
        f"the one that honestly represents your agreement. If your agreement is "
        f"genuinely framework-level, do not invent claim-specific anchors to "
        f"satisfy the form — general convergence is the right move. Up to {cap} "
        f"convergence points total across both forms.\n"
        "\n"
